...
if __name__ == "__main__":
    version=2
    num_improvement_iters=1
    iteration=0
    monitoring_thread=start_monitoring()
    while(iteration<num_improvement_iters):
        #pr = cProfile.Profile()
        #pr.enable()
        ...
        if(mode==1):
            if(iteration==0):
                file_name="agent_v2.0.pt"
            else:
                file_name="agent_last.pt"
            agent.load_state_dict(torch.load(f"{state_path}{file_name}",map_location=device))
        ...
        last10=[]
        last50=[]
        agent_pool=['agent_last.pt', 'agent_v2.0.pt', 'agent_v2.1.pt']
        #if(version>(len(agent_pool)-1)):
        #    agent_pool.append(f'agent_v1.{version-1}.pt')
        pool_index=0
        """ TRAINING LOGIC """
        # train for n number of episodes
        for episode in range(total_episodes):
            if(mode==1):
                agent2.load_state_dict(torch.load(f"{state_path}{agent_pool[pool_index]}",map_location=device))
            # collect an episode
            with torch.no_grad():
            ...
            pool_index=(pool_index+1)%len(agent_pool)
        ...
        if(render):
            if(fixed):
                env = CustomEnv(battle_format=format,team=team1,render_mode="human")
            else:
                env = CustomEnv(battle_format=format,render_mode="human")
        ...
        with torch.no_grad():
            ...
            if(render):
                for episode in range(num_render_episodes):
                    ...
                    while not any(terms) and not any(truncs):
                        ...
                        waiting=[False,False]
                        if(obs[env.battle1.player_username]["wait"]):
                            waiting[0]=True
                        elif(obs[env.battle2.player_username]["wait"]):
                            waiting[1]=True

                        obs, mask = batchify_obs(obs=obs, device=device, mode=mode)
                        if(mode==1):
                            obs2=(obs[1],mask[1])
                            obs=obs[0]
                            mask=mask[0]
            ...
            if(mode==0):
                ...
                bot=AlgoPlayer(state_path=state_path, informat=format,inteam=team1, filename="agent.pt")
